...
ButterworthFilter::ButterworthFilter(void)
{
    this->history1 = 0.f;
    this->history2 = 0.f;
    this->history3 = 0.f;
...
void ButterworthFilter::SetSampleRate(float fs)
{
...
void ButterworthFilter::Set(float cutoff, float q)
{
...
float ButterworthFilter::Run(float input)
{
...
void ButterworthFilter::processBlock(const AudioSourceChannelInfo& bufferToFill)
{
    ...
    {
        buffer->setSample(0, startSample + s, Run(buffer->getSample(0, startSample + s)));
    }
}